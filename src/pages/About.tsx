    ...
    <>
      ...
      <section className="mx-auto max-w-7xl px-6 py-20 lg:px-10">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {values.map((v) => (
            <div key={v.title} className="group relative overflow-hidden rounded-2xl border border-border/60 bg-card/80 backdrop-blur p-8 transition-all hover:border-primary/60 hover:-translate-y-1 hover:bg-card/90 hover:shadow-lg">
              <div className="inline-flex h-12 w-12 items-center justify-center rounded-xl bg-primary/20 text-primary transition-colors group-hover:bg-gold-gradient group-hover:text-primary-foreground">
                <v.icon className="h-5 w-5" />
              </div>
              <h3 className="mt-6 font-display text-xl font-semibold text-foreground">{v.title}</h3>
              <p className="mt-3 text-sm leading-relaxed text-foreground/90">{v.body}</p>
            </div>
          ))}
        </div>
      </section>
